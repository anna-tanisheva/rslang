...
export const BASE_URL = 'https://react-rslang-be-wkho.onrender.com';
const ID = '';
...
export async function fetchPostOrPutUserWord({
  ...
  if (
    (modifyedUserWord?.optional.audiocall.countGames === 1 &&
      modifyedUserWord?.optional.sprint.countGames === 0) ||
    (modifyedUserWord?.optional.sprint.countGames === 1 &&
      modifyedUserWord?.optional.audiocall.countGames === 0)
  ) {
    ...
    if (appState.viewsStates.textbook.mode === 'textbook') {
      ...
      textbookState.words.forEach((oldWord, i) => {
        if (oldWord.id === newWord.id)
          textbookState.words[i] = JSON.parse(JSON.stringify(newWord));
      });
